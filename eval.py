...
def evaluate_gebd(gt_dict, pred_dict):
...
def do_eval(gt_dict, pred_dict, head=None, threshold=0.05):
    """
    Evaluates predicted temporal boundaries against ground-truth annotations.

    Args:
        gt_dict (dict):
            Ground-truth annotations loaded from the provided JSON file.

            Example:
                gt_dict = json.load(open("gt_val.json"))

        pred_dict (dict):
            Dictionary mapping video identifiers to a list of predicted
            temporal boundary timestamps.

            Example:
                pred_dict = {
                    "video_001": [0.12, 0.35, 0.67],
                    "video_002": [0.22, 0.51]
                }

        head (str, optional):
            Identifier specifying which annotation head or subset of the
            ground-truth annotations to evaluate. If None, all annotations
            are evaluated.

            Example:
                head = "action_boundary"

        threshold (float, optional):
            Temporal tolerance used to determine whether a predicted boundary
            matches a ground-truth boundary.
    ...
    Returns:
        dict:
            Dictionary containing evaluation metrics (e.g., precision, recall,
            and F1-score) computed over the evaluated videos.
